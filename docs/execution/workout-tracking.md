...
|              | `useActiveWorkout` (`src/composables/useActiveWorkout.ts`)                                                                                                        | `useWorkoutTracker` (`src/composables/useWorkoutTracker.ts`)                                               |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
...
| Key exports  | `startWorkout`, `finishWorkout`, `restoreActiveWorkout`, `discardWorkout`, `logCalculatorSet`                                                                     | `rebuildOrRestore`, `completeSet`/`toggleSet`, `proposalFor`, `applyProposal`                              |

**Start**: `startWorkout(routineId?)` loads the routine + exercises, calls `prescribeWorkout` ([[prescription-pipeline]]) for slot-aligned prescriptions, captures the current bodyweight **once per session** (prescriptions were rendered against it, so all in-session [[concepts#Bodyweight offset|offset]] math reuses it — see [[bodyweight]]), creates an empty `Workout`, and opens the sheet. A prescription failure is swallowed — a workout can always start. The tracker then builds one card per slot, prefilling rows from `PrescribedSet`s.

**UI shell**: `WorkoutBottomSheet.vue` — a minimizable `AppBottomSheet` with live duration and a rest stopwatch that resets on each completed set, paging between `WorkoutTrackerPanel` and `WorkoutCalculatorPanel`. Set rows (`WorkoutSetRow.vue`) keep weights in kg internally and validate on complete; any finite added weight is valid — `0` (plain bodyweight) and negatives (assistance) included.
...
An in-progress workout survives app death via a localStorage snapshot (`src/composables/workoutPersistence.ts`): key `yafa:activeWorkout`, versioned (`SNAPSHOT_VERSION` currently 2 — bump it on breaking shape changes; purely additive optional fields, like the session `bodyweightKg`, don't need a bump since restore falls back gracefully). The snapshot embeds everything needed to resume: workout, routine, exercises map, **slot-aligned** prescriptions, planned counts, calculator sets, cards, session bodyweight, and UI state.

Write path: the tracker debounces `persist()` at 400 ms on any card change and flushes synchronously on `visibilitychange`/`pagehide`. Read path: `restoreActiveWorkout` (`useActiveWorkout.ts`) runs in `main.ts` _before_ the app mounts, rehydrates all refs, and hands the tracker its cards through a one-shot `takePendingRestore`. The snapshot is device-only — deliberately excluded from backups ([[backup-restore#What travels in a backup|backup-restore]]) — and cleared on finish/discard.

## Finish ordering

`finishWorkout` (`useActiveWorkout.ts`) runs a deliberate sequence:
...
`WorkoutCalculatorPanel.vue` is a routine-agnostic tool inside the running workout: solve any of weight/reps/RPE from the other two (`solveWeight`/`solveReps`/`solveRpe`, `src/engine/calculator.ts` — same matrix as prescriptions, so results agree), a live effective e1RM (`liveEffectiveE1rm`, `src/engine/state.ts` — the mid-session mirror of the catch-up), and an optional fatigue toggle (`computeFatigueAdjustment`). For bodyweight exercises the solvers run in total-load space and results convert back to added weight ([[bodyweight]]); the numeric keypad's **±** key enters assistance weights. Sets logged here are kept separate (`calculatorSets`) and merged into the workout only at finish.

## Key functions

| Function                          | File                                    | Note                                |
| --------------------------------- | --------------------------------------- | ----------------------------------- |
| `startWorkout`                    | `src/composables/useActiveWorkout.ts`   | Load → prescribe → open sheet       |
| `finishWorkout`                   | `src/composables/useActiveWorkout.ts`   | Summary before persist before fold  |
| `restoreActiveWorkout`            | `src/composables/useActiveWorkout.ts`   | Pre-mount snapshot rehydration      |
| `proposeSetAdjustment`            | `src/engine/adjustment.ts`              | Pure proposal; never touches c1RM   |
| `proposalFor`                     | `src/composables/useWorkoutTracker.ts`  | Green-dot surfacing rule            |
| `applyProposal`                   | `src/composables/useWorkoutTracker.ts`  | Confirmed apply + card-only cascade |
| `fillColdStartFromGovernor`       | `src/composables/useWorkoutTracker.ts`  | Auto cold-start fill                |
| `read/write/clearWorkoutSnapshot` | `src/composables/workoutPersistence.ts` | Versioned localStorage snapshot     |
| `liveEffectiveE1rm`               | `src/engine/state.ts`                   | Calculator's live anchor estimate   |
